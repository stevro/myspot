    ...
    >
      ...
      <v-card-actions>
        ...
        <v-row v-if="isSaved">
          ...
          <v-col cols="12">
            <v-btn :to="{name:'recurring-events'}" block>
...
async function getEvent() {
  ...
  const snapshot = await getDoc(doc(firestore, "recurrent_events", route.params.eventId).withConverter(recurrentEventConverter));
...
async function cancel() {
  router.push({name: 'recurring-events'})
}
...
async function submitEvent() {
  try {
    ...
    const docRef = (await setDoc(doc(firestore, "recurrent_events", spotEvent.value.id).withConverter(recurrentEventConverter), spotEvent.value));
